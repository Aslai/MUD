...
DLL_EXPORT void Lua::Load(std::string block, std::string name, int flags){
    size_t pos = block.length();
    errorlevel = luaL_loadbuffer( L, block.c_str(), pos, name.c_str() );
}
DLL_EXPORT void Lua::Load(std::vector<char> block, std::string name, int flags){
    size_t pos = block.size();
    errorlevel = luaL_loadbuffer( L, &(block[0]), pos, name.c_str() );
}
DLL_EXPORT void Lua::Load( std::string fname ){
    errorlevel = luaL_loadfile(L, fname.c_str() );
}
DLL_EXPORT void Lua::Load( Lua::Script script ){
    Load( script.Data, script.Title, 0 );
}

DLL_EXPORT Lua::Lua( std::string block, std::string name, int flags ){
    Init();
    Load( block, name, flags );
}
DLL_EXPORT Lua::Lua( std::vector<char> block, std::string name, int flags ){
    Init();
    Load( block, name, flags );
}
DLL_EXPORT Lua::Lua( std::string fname ){
    Init();
    Load( fname );
}
DLL_EXPORT Lua::Lua( Lua::Script script ){
    Init();
    Load( script );
}
...
DLL_EXPORT std::vector<char> Lua::Dump() const{
...
DLL_EXPORT Lua::Function Lua::GetFunction( const char* name ) const{
    return Function(L, name);
}

DLL_EXPORT void Lua::Run(){
    if (errorlevel == 0)
