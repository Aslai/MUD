...
DLL_EXPORT Lua::Stack::Stack(){
    position = 1;
}

DLL_EXPORT int Lua::Stack::trythrow(int idx){
...
DLL_EXPORT int Lua::Stack::lua_push( std::string s ){
    lua_pushstring( L, s.c_str() );
    return 0;
}

DLL_EXPORT int Lua::Stack::lua_push( const char* s ){
    lua_pushstring( L, s );
    return 0;
}

DLL_EXPORT int Lua::Stack::lua_push( int s ){
    lua_pushinteger( L, s );
    return 0;
}

DLL_EXPORT int Lua::Stack::lua_push( unsigned int s ){
    lua_pushunsigned( L, s );
    return 0;
}

DLL_EXPORT int Lua::Stack::lua_push( double s ){
    lua_pushnumber( L, s );
    return 0;
}

DLL_EXPORT int Lua::Stack::lua_push( void ){
    return 0;
}

DLL_EXPORT int Lua::Stack::lua_push( void* s ){
    lua_pushnumber( L, (long long)s );
    return 0;
}

DLL_EXPORT int Lua::Stack::lua_push( Lua::Value s ){
    switch( s.myType ){
...
DLL_EXPORT std::string Lua::Stack::lua_ret( std::string, int pos ){
...
DLL_EXPORT const char* Lua::Stack::lua_ret( const char*, int pos ){
    if( pos == -100000 ) pos = position++;
    if( trythrow(pos) == 0 )
...
DLL_EXPORT int Lua::Stack::lua_ret( int , int pos ){
    if( pos == -100000 ) pos = position++;
    if( trythrow(pos) == 0 ){
        try{
...
DLL_EXPORT double Lua::Stack::lua_ret( double, int pos ){
    if( pos == -100000 ) pos = position++;
    if( trythrow(pos) == 0 )
        try{
...
DLL_EXPORT void* Lua::Stack::lua_ret( void*, int pos ){
    if( pos == -100000 ) pos = position++;
    if( trythrow(pos) == 0 ){
        try{
...
DLL_EXPORT Lua::Function Lua::Stack::lua_ret( Lua::Function, int pos ){
    if( pos == -100000 ) pos = position++;
    if( trythrow(pos) == 0 ){
        lua_pushvalue( L, pos );
...
DLL_EXPORT Lua::Table Lua::Stack::lua_ret( Lua::Table, int pos ){
    if( pos == -100000 ) pos = position++;
    if( trythrow(pos) == 0 ){
        lua_pushvalue( L, pos );
        int f = luaL_ref(L, LUA_REGISTRYINDEX);
...
DLL_EXPORT Lua::Value Lua::Stack::lua_ret( Lua::Value, int pos ){
    if( pos == -100000 ) pos = position++;
    if( trythrow(pos) == 0 ){
