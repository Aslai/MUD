...
namespace MUD{
    ...
    public:
        ...
        enum class ANSICodes{
            ...
            SelectGraphicRendition,         ///<Format: __SelectGraphicRendition__ [ _n_ ]\n
                                            ...
                                            ///<See MUD::Telnet::SGRCodes for possible values and explanations for their behavior.
        ...
        public:
            ...
            friend TheScreen;
                ...
                bool wraps;
            public:
                DLL_EXPORT TheCursor( TheScreen& screen );
                DLL_EXPORT Error ShouldWrap( bool shouldWrap = true );
                DLL_EXPORT Error Advance( int amount = 1 );
                DLL_EXPORT Error LineFeed( int amount = 1 );
                DLL_EXPORT Error CarriageReturn();
                DLL_EXPORT Error MoveTo( int x, int y );
                DLL_EXPORT Error Hide();
                DLL_EXPORT Error Unhide();
                DLL_EXPORT bool Hidden();
            };
            friend TheCursor;
        private:
            int width, height;
        ...
        public:
            DLL_EXPORT TheCursor &Cursor();
            DLL_EXPORT TheScreen( int Width, int Height, TelnetSessionInternal &Parent );
            DLL_EXPORT int Width();
            DLL_EXPORT int Height();
            DLL_EXPORT Error Clear();
            DLL_EXPORT Error SetTerminal( std::string TerminalType );
            DLL_EXPORT std::string GetTerminal();
            DLL_EXPORT Error Resize( int w, int h );
            DLL_EXPORT Error SetColor( Color foreground, Color background );
        };
    ...
    private:
        ...
        public:
			DLL_EXPORT TheScreen& Screen();
            DLL_EXPORT TelnetSession( CommStream s, Telnet &Parent);
            DLL_EXPORT ~TelnetSession();
            DLL_EXPORT Error SendLine( std::string line );
            DLL_EXPORT Error SendChar( const char c );
            DLL_EXPORT Error SendCommand( Telnet::Commands cmd1 = Telnet::Commands::NONE, Telnet::Commands cmd2 = Telnet::Commands::NONE );
            DLL_EXPORT Error SendSubnegotiation( Telnet::Commands cmd, char* data = nullptr, size_t len = 0 );
            DLL_EXPORT Error SendSubnegotiation( Telnet::Commands cmd1, Telnet::Commands cmd2, char* data = nullptr, size_t len = 0 );

            DLL_EXPORT bool HasLine();
            DLL_EXPORT bool HasChar();
            DLL_EXPORT std::string ReadLine();
            DLL_EXPORT char ReadChar();
            DLL_EXPORT std::string PeekLine();
            DLL_EXPORT char PeekChar();
			DLL_EXPORT Error Disconnect( bool remoteFailure = false );
...
void callback( MUD::Telnet::TelnetSession session ){
...
****************************************/
        DLL_EXPORT Error Listen( int port, std::function<void(TelnetSession)> callback );
...
****************************************/
        DLL_EXPORT Error ReadTerms( std::string fname );

        DLL_EXPORT Error Disable( Feature f );

        DLL_EXPORT Error Enable( Feature f );

        DLL_EXPORT Error Disconnect();
